...
$RAM_GB = 128
$RAM_GB_AUTOMATION_HOST = 192
$RAM_GB_RESERVATION = 0 # set to $RAM_GB for VCF mgmt domain hosts
$CPU = 32
$CoresPerCPU = 8
$NUM_ADD_NICS = 1
$USE_VSAN_OSA = $false
...
$DataDiskSizeGB = 200 
$ReservationMHz = 0 # these hosts don't usually need CPU reservation for bringup to succeed
$webInstallHost = '10.41.0.250'
$DrsClusterGroupName = "" # an existing group that prevents vESX VMs from running on specific hosts
$iterationList = (11,12,13)
...
foreach ( $iteration in $iterationList ) {
    ...
    Set-VMResourceConfiguration -Conf $conf -CpuReservationMhz $ReservationMHz -MemReservationGB $RAM_GB_RESERVATION
