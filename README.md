...
This project is evolving form a need to create standardized environments for VCF and VVF test cases. The focus is on the "Day 0" (and "Day -1") configuration of the underlying compute components to facilitate the installation of VCF or VVF to satisfy the requirements of the testing.

The initial script creates the VMs needed to install ESX in a test lab since getting all of the parts correct was tedious, especially when nesting vSAN ESA's required virtual NVMe devices. It has evolved to using an http-based build server to stream the ESX image and kickstart it by passing initial boot values into UEFI and powering up the VMs. An example of that build server may be added here since I have found it immensely valuable.
...
There are two main configurations: "blue" and "green" to represent two virtually identical but separate environments -- one will be "live" while the other will be for test/dev or early access. To that end, the "blue" and "green" environments in project create two-site configurations: 
