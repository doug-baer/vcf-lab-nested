## Updated May 8, 2026 - cleanup (same as build management hosts but with different CPU, RAM, disk sizes and host numbers)
...
$DataDiskSizeGB = 600 
$NUM_DATA_DISKS = 3
$ReservationMHz = 0 # these hosts generally do not need CPU reservation
$webInstallHost = '10.41.0.250'
$DrsClusterGroupName = "" # an existing host group that prevents the vESX VMs from running on specific hosts
...
foreach ( $iteration in $iterationList ) {
    ...
    # Add new disk on SCSI (will move to NVMe Controller later)
    if( $USE_VSAN_OSA ) {
